...
if(Test-Path $path\CDPInfo.xlsx){
    Remove-Item $path\CDPInfo.xlsx
}

###host name to be changed
$hostNames = Get-Content $path\hostList.txt

foreach($hostName in $hostNames){
$VMhost = $hostName.Replace(" ","")
if($VMhost -like "*.mud.internal.co.za*"){
}else{
    $VMhost += ".mud.internal.co.za"
}

$VMhost

      $Esxihosts = Get-VMHost $VMhost | Where-Object {$_.ConnectionState -like "Connected" -or "Maintenance"}    
      ...
      $Collection | Export-Excel -Path "$path\CDPInfo.xlsx" -BoldTopRow -AutoSize
}